import { functionKeys } from "../InputListeners.js"
import { resetSettingsToDefault } from "../settings/Settings.js"
import { DomHelper } from "../ui/DomHelper.js"
import { ConfirmDialog } from "./ConfirmDialog.js"
import { getKeyBinder } from "./KeyBinder.js"
import { Notification } from "./Notification.js"
import { createListGroupDiv, hideAllDialogs } from "./UI.js"
/**
...
export class SettingUI {
	...
	createSettingTabContentDiv(tabName, settingGroups) {
		...
		if (tabName == "General") {
			let btnGrp = DomHelper.createSettingsButtonGroup([
				this.getKeyToKeyboardBindingButton(),
				this.getShowHotKeysButton(),
				this.getClearKeyBindingButton(),
				this.getResetButton()
			])
	...
	static createListSettingDiv(setting) {
		let el = DomHelper.createInputSelect(setting)
	...
	static createCheckboxSettingDiv(setting) {
		let el = DomHelper.createCheckbox(setting)
	...
	static createTextSettingDiv(setting) {
		let el = DomHelper.createTextInputDiv(setting)
		el.classList.add("settingContainer")
		setting.setUIValue = val => (el.querySelector("input").value = val)
		return el
	}
	static createLabel(setting) {
		let el = DomHelper.createDiv({}, { innerHTML: setting.label })
		el.classList.add("settingContainer")
		el.classList.add("label")
		return el
	}
	static createSliderSettingDiv(setting) {
		let el = DomHelper.createSliderWithLabelAndField(setting).container
		el.classList.add("settingContainer")
