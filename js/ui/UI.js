...
import { getRenderer, PROGRESS_BAR_CANVAS_HEIGHT } from "../Rendering/Render.js"
import { Notification } from "./Notification.js"
import { ElementHighlight } from "./ElementHighlight.js"
import { saveSongInDb } from "../settings/IndexDbHandler.js"
import { initThreeJs } from "../Rendering/ThreeJs/threeJsHandler.js"
import { getRenderDimensions } from "../Rendering/RenderDimensions.js"
...
export class UI {
		...
		document.querySelectorAll(".innerMenuDiv").forEach(el => {
			...
			let top =
				this.getCloseInnerMenuDivButton().clientHeight > 0
					? this.getCloseInnerMenuDivButton().clientHeight + 0 + "px"
	...
	setExampleSongs(exampleSongsJson, loadRandomStartingSong) {
		this.songUI.setExampleSongs(exampleSongsJson, loadRandomStartingSong)
	}
	setDbSongs(dbSongs) {
	...
	getVolumneButtonGroup() {
		...
		let volSlider = this.getMainVolumeSlider()
		volSlider.label.appendChild(this.getMuteButton())
		DomHelper.appendChildren(volumeGrp, [volSlider.container])
		return volumeGrp
	}
	...
	readFile(file, name) {
		...
		reader.onload = async function (theFile) {
			await getPlayer().loadSong(reader.result, {
				fileName,
				name: name || fileName
			})
			if (getSetting("saveIndexedDb")) {
				let midiData = reader.result
				if (!this.songUI.songDivs.hasOwnProperty(fileName)) {
					saveSongInDb({ fileName, midiData, trackSettings: getTracks() })
				}
			}
		}.bind(this)
	...
	getSpeedUpButton() {
		if (!this.speedUpButton) {
			...
			this.speedUpButton.id = "speedField"
	...
	getMainVolumeSlider() {
		if (!this.mainVolumeSlider) {
			...
			this.mainVolumeSlider = DomHelper.createSliderWithLabel(mainVolSetting)
		}
	...
	getMidiSetupDialog() {
		if (!this.midiSetupDialog) {
			...
			let devicesDiv = DomHelper.createDivWithClass("row")
			...
			devicesDiv.appendChild(this.inputDevicesDiv)
			devicesDiv.appendChild(this.outputDevicesDiv)
			this.midiSetupDialog.appendChild(devicesDiv)
		}
	...
	getSandwichFullscreenButton() {
		if (!this.swFullscreenButton) {
			...
			DomHelper.addClassToElement("floatSpanLeft", this.swFullscreenButton)
			let fullscreenSwitch = () => (this.fullscreen = !this.fullscreen)
			document.body.onfullscreenchange = fullscreenSwitch.bind(this)
		}
	...
	getSandwichSettingsButton() {
		if (!this.swSettingsButton) {
			...
			DomHelper.addClassToElement("floatSpanLeft", this.swSettingsButton)
	...
	getSandwichMuteButton() {
		...
		DomHelper.addClassToElement("floatSpanLeft", this.swMuteButton)
...
export const createListGroupDiv = (listName, itemList, createDivFunc) => {
	...
	let label = DomHelper.createElementWithClass(
		"settingsLabel settingsGroupLabel clickableTitle",
		"div",
		{},
		{}
	)
