...
import { getTrackColor, isTrackSheetEnabled } from "../player/Tracks.js"
import { PROGRESS_BAR_CANVAS_HEIGHT } from "../Rendering/Render.js"
import { getRenderDimensions } from "../Rendering/RenderDimensions.js"
import { getSetting, triggerSettingCallback } from "../settings/Settings.js"
import {
	formatNote,
...
export class SheetRender {
	...
	getMeasuresToDraw(sheetGen, curPlayMeasureIndex, cursorX) {
		const renderDims = getRenderDimensions()
		let { firstDrawnMeasure, firstDrawnMeasureIndex } =
	...
	getFirstDrawnMeasure(sheetGen, curPlayMeasureIndex, cursorX) {
		...
		return { firstDrawnMeasure, firstDrawnMeasureIndex }
	}
	getCursorXPosition(
		sheetGen,
		curPlayMeasure,
	...
	computeStaffYs(sheetGen) {
		...
		if (this.ctx.canvas.height != Math.floor(curY * this.scale)) {
			...
			this.ctx.canvas.style.bottom =
				-this.ctx.canvas.getBoundingClientRect().height -
