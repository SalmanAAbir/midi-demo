...
import { getLoopManager, getCurrentLoop } from "../LoopManager.js"
...
class Player {
	...
	setScrollAim(seconds) {
		this.scrollAim = seconds
	}
	setTime(seconds) {
	...
	playTick() {
		...
		let loopEnabled = getSetting("enableLoop")
		let autoLoop = getSetting("autoLoop")
		let muteLoop = getSetting("muteLoop")
		let curLoop = getCurrentLoop()

		if (this.scrollAim != null) {
			let diff = (currentTime - this.scrollAim) * 0.1
			if (diff < 0.001) {
				this.setTime(this.scrollAim)
				this.scrollAim = null
				this.requestNextTick()
				return
			}
			this.setTime(currentTime - diff)
			this.requestNextTick()
			return
		}

		if (autoLoop && loopEnabled && this.loopDelay > 0) {
			this.loopDelay -= delta
			if (this.loopDelay <= curLoop.delay && !this.isLoopReset) {
				this.isLoopReset = true
				this.setScrollAim(curLoop.start - 0.1)
			}
			if (this.loopDelay <= 0) {
				this.loopDelay = 0
				this.resume()
		...
		if (autoLoop && loopEnabled && curLoop && this.scrollAim == null) {
			if (curLoop.end < currentTime + delta) {
				this.isLoopReset = false
				this.pause()
				this.loopDelay = curLoop.delay + 1
				this.requestNextTick()
				return
			} else if (currentTime < curLoop.start - 0.5) {
				this.setScrollAim(curLoop.start - 0.1)
				this.requestNextTick()
				return
			}
		...
		while (this.isNextNoteReached(currentTime)) {
			...
			let note = this.noteSequence[0]
			if (
				this.noteSequence[0] &&
				(!isTrackRequiredToPlay(note.track) ||
					this.isInputKeyPressed(note.noteNumber))
			) {
				if (
					!loopEnabled ||
					!curLoop ||
					!muteLoop ||
					(curLoop.end * 1000 > note.timestamp + 50 &&
						curLoop.start * 1000 <= note.timestamp + 50)
				) {
	...
	isNextNoteReached(currentTime) {
		...
		return (
			...
			note.timestamp / 1000 < currentTime + lookahead * this.playbackSpeed
		)
	}
