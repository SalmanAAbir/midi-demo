...
import { SheetRender } from "../sheet/SheetRender.js"
import { createKeyBinder, getKeyBinder } from "../ui/KeyBinder.js"
import { threeJsRender } from "./ThreeJs/threeJsHandler.js"
import { SHEET_SIDE_MARGIN } from "../sheet/SheetGenerator.js"
...
export class Render {
	constructor() {
		...
		this.buttonFontColor = getCssVariable("buttonFontColor")

		this.setupCanvases()

		initProgressBarRenderer(this.progressBarCtx)
	...
	render() {
		...
		if (!playerState.loading && playerState.song) {
			...
			getProgressBarRender().render(
				this.progressBarCtx,
				time,
				end,
				playerState.song.markers
			)
	...
	getNoteRenderInfo(note, time) {
		time *= 1000
		let noteDims = getRenderDimensions().getNoteDimensions(time, note)
	...
	drawBPM(playerState) {
		this.renderText(Math.round(playerState.bpm) + " BPM", 20, this.textY)
	}
	drawFPS() {
		let thisTimestamp = window.performance.now()

		let timePassed = thisTimestamp - this.lastTimestamp
		this.frameTime += (timePassed - this.frameTime) / this.fpsFilterStrength
		this.renderText((1000 / this.frameTime).toFixed(0) + " FPS")
	...
	drawLoop() {
		this.renderText(
			"Looping Measure #" +
				getSetting("loopStart") +
				" - #" +
				getSetting("loopEnd")
		)
		let txtMetrics = this.ctx.measureText("M").width
		this.textY += 0.18 * txtMetrics
	}
	renderText(txt) {
		const { ctx } = this
		...
		let txtMetrics = ctx.measureText("M").width
		ctx.fillText(txt, 20, this.textY)

		this.textY += 2 * txtMetrics
	}
	getTextY() {
		return (
	...
	setupCanvases() {
		...
		this.getProgressBarCanvas().style.bottom =
		...
		this.getSheetCanvas().style.bottom =
			-sheetHeight /* / window.devicePixelRatio*/ -
			PROGRESS_BAR_CANVAS_HEIGHT +
			"px"
		this.setFgCtxZIndex()
		this.setCtxBlur()
