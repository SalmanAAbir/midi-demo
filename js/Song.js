		...
		Object.keys(this.temporalData.beatsBySecond).forEach(second => {
			this.temporalData.beatsBySecond[second].forEach(beat => {
				...
				if (beat[0] < this.getEnd()) {
					...
					counter = Math.floor(beat[1] / beatsToSkip) + 1
					this.measureLines[beatSecond].push([beat[0] - adjust, counter])
				}
			})
		})
		let end = this.getEnd()
		if (!this.measureLines.hasOwnProperty(Math.floor(end))) {
			this.measureLines[Math.floor(end)] = []
		}
		this.measureLines[Math.floor(end)].push([end, counter + 1])
	}
	setSustainPeriods() {
		this.sustainPeriods = []
