import { getCurrentLoop, getLoopManager } from "./LoopManager.js"

import { getCurrentSong, getPlayer, START_DELAY } from "./player/Player.js"
import {
	getProgressBarTime,
	setProgressBarGrabbed
} from "./Rendering/ProgressBarRender.js"
import { getRenderer } from "./Rendering/Render.js"
import { getRenderDimensions } from "./Rendering/RenderDimensions.js"
import { getSetting, setSettingUI } from "./settings/Settings.js"
import { getKeyBindings } from "./ui/KeyBinder.js"
	...
	onMouseDownProgressCanvas(render) {
		return ev => {
			...
			if (ev.target == render.getProgressBarCanvas()) {
				...
				getPlayer().pause()
				let newTime = getProgressBarTime(ev.clientX)
...
export const functionKeys = {
	...
	L: {
		...
		func: e => {
			e.preventDefault()
			setSettingUI("enableLoop", !getSetting("enableLoop"))
		}
	},
	P: {
		key: "P",
		name: "P",
		description: "Advance loop by 1 measure",
		func: e => {
			e.preventDefault()
			getLoopManager().moveForward()
		}
	},
	O: {
		key: "O",
		name: "O",
		description: "Move loop back by 1 measure",
		func: e => {
			e.preventDefault()
			getLoopManager().moveBackward()
		}
	},
	U: {
		key: "U",
		name: "U",
		description: "Expand loop start",
		func: e => {
			e.preventDefault()
			getLoopManager().expandLeft()
		}
	},
	I: {
		key: "I",
		name: "I",
		description: "Expand loop end",
		func: e => {
			e.preventDefault()
			getLoopManager().expandRight()
		}
	},
	K: {
		key: "K",
		name: "K",
		description: "Decrease loop start",
		func: e => {
			e.preventDefault()
			getLoopManager().shrinkLeft()
		}
	},
	J: {
		key: "J",
		name: "J",
		description: "Decrease loop end",
		func: e => {
			e.preventDefault()
			getLoopManager().shrinkRight()
		}
	},
	Z: {
		key: "Z",
		name: "Z",
		description: "Jump to loop start",
		func: e => {
			e.preventDefault()
			getPlayer().setScrollAim(getCurrentLoop().start - 0.1)
		}
	}
}
